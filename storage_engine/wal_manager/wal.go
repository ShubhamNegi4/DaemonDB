...
/*
Wal Manager writes each operation performed on disk
It is used by Storage Engine to perform WAL recovery along with the help of checkpoint manager
This file contains the functions to read/write the WAL records, and operations on WAL entries
...
WALManager.AppendOperation — builds a WAL record from an Operation,
calls Segment.Append to write it, advances CurrentLSN by bytes written. Still no fsync — data in OS buffer only.

WALManager.Sync — calls Segment.Sync to fsync, then sets FlushedLSN = CurrentLSN.
This is the durability boundary. After this, buffer pool is allowed to flush pages whose LSN <= FlushedLSN.


CurrentLSN tracks what's been written (to OS buffer).
FlushedLSN tracks what's been synced (to disk).

The flow is:
AppendOperation/AppendToBuffer    — write to OS buffer, advance CurrentLSN
        ↓
    Sync()                        — fsync OS buffer → disk, set FlushedLSN
        ↓
BufferPool.FlushAllPages          — now safe, pg.LSN <= FlushedLSN
*/
...
// AppendToBuffer writes WAL record using pre-allocated LSN, no fsync
func (wm *WALManager) AppendToBuffer(op *types.Operation, lsn uint64) error {
	wm.mu.Lock()
	defer wm.mu.Unlock()
	...
	// Correct CurrentLSN to actual position after real write
	n, err := wm.CurrSegment.Append(encodedRecord)
	if err != nil {
		return err
	}

	fmt.Printf("[WAL] AppendToBuffer lsn=%d type=%d txnID=%d table=%s\n", lsn, op.Type, op.TxnID, op.Table)

	wm.CurrentLSN += uint64(n)
	return nil
}

// AppendOperation keeps existing behavior for txn control records (BEGIN/COMMIT/ABORT)
// These still sync immediately
...
func (wm *WALManager) Sync() error {
	...
	fmt.Printf("[WAL] Sync — flushing to disk, currentLSN=%d\n", wm.CurrentLSN)
	if err := wm.CurrSegment.Sync(); err != nil {
		return err
	}
	wm.FlushedLSN = wm.CurrentLSN // everything up to now is durable
	fmt.Printf("[WAL] Sync complete — flushedLSN=%d\n", wm.FlushedLSN)
	return nil
}

func (wm *WALManager) AllocateLSN(dataSize int) uint64 {
	wm.mu.Lock()
	defer wm.mu.Unlock()
	lsn := wm.CurrentLSN
	fmt.Printf("[WAL] AllocateLSN lsn=%d estimatedSize=%d\n", lsn, dataSize)
	wm.CurrentLSN += uint64(RecordHeaderSize + dataSize)
	return lsn
}
